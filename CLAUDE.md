...
├── content-management/        # Blog, announcements, media, home builder
...
**IMPORTANT**: Functions use **1st Gen** (firebase-functions v1). Always import from `firebase-functions/v1`:
```typescript
import * as functions from 'firebase-functions/v1';
// NOT: import * as functions from 'firebase-functions'; (resolves to v2 in firebase-functions 7.x)
...
**Key Functions** (all in `southamerica-east1`):
...
**File**: `firestore.rules` (~460 lines, compact role-based)

**Two-layer permission model**:
- **Firestore rules** = security barrier (auth, approved status, admin, ownership)
- **App-side PermissionService** = business logic (modules, actions, granular roles)

**Helper functions in rules**:
- `isAuthenticated()` - Check if user is logged in
- `getUserData()` - Get current user document
- `isApproved()` - Check user is authenticated AND status is `approved`
- `hasRole(role)` - Check specific role
- `isAdmin()` - Check admin role
- `isOwner(userId)` - Check if field matches current user
- `isDocOwner()` - Check if document's `userId` matches current user

**Collection access patterns**:
- Public read: `events`, `blogPosts`, `devotionals`, `publicPageSettings`
- Auth user interact: `prayerRequests`, `forumTopics`, `forumReplies`
- Approved users: `members`, `departments`, `transactions`, `projects`, etc.
- Owner-based: `notifications` (own only)
- Admin-only: `settings`, `logs`, `backups`

## UX Patterns

### Notifications (Toast)

**NEVER use `alert()`, `window.confirm()`, or `window.prompt()`**. Use:
...
The `<Toaster>` and `<ConfirmDialogProvider>` are set up in `App.tsx`.
...
1. **React Context API** - For app-wide state
   ...
   - `SettingsContext` - App settings (theme, colors, payment info)
   - `NotificationContext` - In-app notification bell
   - `ConfirmDialogContext` - Modal confirmations and prompts
...
1. Add function in `functions/src/`
2. **Use `firebase-functions/v1` import** (NOT default import)
3. Set region to `southamerica-east1`
4. Use Firebase Admin SDK for privileged operations
...
8. **firebase-functions v7+**: Default import resolves to v2 API. Always use `import * as functions from 'firebase-functions/v1'` for existing 1st Gen functions. Do NOT upgrade to 2nd Gen in-place (not supported by Firebase).

9. **No alert/confirm/prompt**: Use `toast` from react-hot-toast for notifications and `useConfirmDialog()` for confirmations. Never use browser native dialogs.

10. **Firestore rules size limit**: Keep rules compact (~460 lines). Granular permission logic belongs in app-side PermissionService, not in Firestore rules.
...
- **firebase-functions 7.x** - Cloud Functions (use v1 import)
